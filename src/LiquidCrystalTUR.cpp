...
LiquidCrystal::LiquidCrystal(uint8_t rs,  uint8_t enable, uint8_t bl, uint8_t cr,
           uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3)
{
  init(1, rs, 255, enable, d0, d1, d2, d3, 0, 0, 0, 0);
  _backlightPin = bl;
...
void LiquidCrystal::createChar(uint8_t location, uint8_t charmap[], bool userAdded) {
    if(userAdded && location<6) return;
